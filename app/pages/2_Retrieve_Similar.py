...
query_features = collect_query_features(schema, cases, key_prefix="retrieve")
st.caption("Symbolic similarities are computed against the structured query above.")
...
for rank, (i, s) in enumerate(zip(idxs, sims), start=1):
    ...
    st.write("Symbolic similarity:", round(float(gscore), 3))
